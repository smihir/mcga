/* New version to handle sigterm  PF 9/27/2003  */
/******************************************************************************/
...
void main(int argc, char *argv[])
{
    ...
    extern void termhandler(int sig);
    
    if (signal(SIGTERM, termhandler) == SIG_ERR) {
        fprintf(stderr, "cannot set handler for TERM\n");
        exit(1);
    }
 ...
 /* This loop is the main loop driver of the HINT kernel.                     */
    ...
    /* Will break out of above loop if memory allocation does not return NOERROR */
    /* Control will also return here from TERM handler */
    memuse = (int)(qdata[i-1].n * (sizeof(RECT)+sizeof(DSIZE)+sizeof(ISIZE)));
    if ( eflag == NOERROR )
      {
	printf("\nReturned from longjmp\n"); fflush(stdout);
	printf("\nThis run exited at %d subintervals -> %d bytes\n", n, memuse); fflush(stdout);
      }
...
{
  /* Handler for TERM signal sent by kernel if memory is too low */
